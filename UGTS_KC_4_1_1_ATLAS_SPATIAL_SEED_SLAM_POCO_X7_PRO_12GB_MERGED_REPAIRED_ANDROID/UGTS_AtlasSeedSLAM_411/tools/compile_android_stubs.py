...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="ugts_android_stub_") as temporary:
        ...
        seednative_classes = temp / "seednative-classes"
        ...
        result = run([
            ...
            "-d", str(seednative_classes),
            *map(str, sorted((ROOT / "seednative/src/main/java").rglob("*.java"))),
        ])
        if result.returncode:
            print(result.stdout + result.stderr)
            return result.returncode
        classpath = f"{stub_classes}:{core_classes}:{seednative_classes}"
        result = run([
        ...
        if result.returncode:
            print(result.stdout + result.stderr)
            return result.returncode
        class_count = len(list(app_classes.rglob("*.class"))) + len(list(seednative_classes.rglob("*.class")))
        print(f"Android shell + native bridge stub compile: PASS ({class_count} class files)")
        return 0
