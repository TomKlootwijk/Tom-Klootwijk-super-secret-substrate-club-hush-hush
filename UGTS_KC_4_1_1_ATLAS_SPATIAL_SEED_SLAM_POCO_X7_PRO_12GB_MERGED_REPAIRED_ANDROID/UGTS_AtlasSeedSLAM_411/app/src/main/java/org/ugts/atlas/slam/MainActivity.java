...
import org.ugts.atlas.seednative.NativeSeedBridge;
...
public final class MainActivity extends Activity
    ...
    private SyntheticFrameSource syntheticFrames;
    ...
    protected void onCreate(Bundle state) {
        ...
        engine.setNativeIntegrityStatus(NativeSeedBridge.status());
        ...
        syntheticFrames = new SyntheticFrameSource(engine, this);
        buildUi();
    ...
    private void buildUi() {
        ...
        finish.setOnClickListener(view -> {
            ...
            syntheticFrames.stop();
            status.setText("Finished · ready to export KSEED");
        });
        scale.setOnClickListener(view -> scaleAction());
        export.setOnClickListener(view -> exportAction());
        demo.setOnClickListener(view -> beginDemo());
    }
    ...
    private void beginDemo() {
        ...
        sensors.stop();
        Seed128 demoSeed = Seed128.derive("UGTS-KC-4.1.1-POCO-DEMO-FIXTURE");
        String id = "atlas411_demo_"
                + new SimpleDateFormat("yyyyMMdd_HHmmss", Locale.ROOT).format(new Date());
        engine.start(id, now, demoSeed, true);
        engine.setNativeIntegrityStatus(NativeSeedBridge.status());
        demoActive = true;
        scaleStart = null;
        banner.setText("DEMO · SYNTHETIC TAG BIT 31 · NOT REAL-WORLD EVIDENCE");
        banner.setBackgroundColor(Color.argb(220, 104, 48, 132));
        status.setText("Deterministic synthetic fixture · 300 frames maximum");
        syntheticFrames.start(demoSeed, now);
    }
    ...
    public void onRequestPermissionsResult(
        ...
        if (requestCode == CAMERA_REQUEST
            ...
            status.setText("Camera permission denied · Demo remains available");
        }
    }
    ...
    public void onCameraError(Throwable error) {
        runOnUiThread(() -> status.setText(
                "Camera unavailable: " + error.getClass().getSimpleName()
                        + " · use Demo for isolated synthetic validation"));
    }
    ...
    public void onSnapshot(SlamSnapshot snapshot, String thermalLabel) {
        runOnUiThread(() -> {
            ...
            if (demoActive) {
                banner.setText("DEMO · SYNTHETIC TAG BIT 31 · NOT REAL-WORLD EVIDENCE");
                banner.setBackgroundColor(Color.argb(220, 104, 48, 132));
            } else if ("metric_anchor".equals(snapshot.scaleState)) {
    ...
    private void exportAction() {
        ...
        status.setText("Encoding KSEED 4.1 evidence stream…");
        exportExecutor.execute(() -> {
            try {
                File file = AndroidSessionExporter.exportKSeed(this, data);
                pendingExport = file;
        ...
        exportExecutor.execute(() -> {
            try (FileInputStream input = new FileInputStream(source);
                ...
                runOnUiThread(() -> {
                    ...
                    Toast.makeText(this, "KSEED session saved", Toast.LENGTH_LONG).show();
                });
            } catch (Exception error) {
                runOnUiThread(() -> {
                    export.setEnabled(true);
