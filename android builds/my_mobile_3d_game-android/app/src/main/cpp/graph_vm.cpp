...
void GraphVm::ready(std::vector<NodeData>& nodes) {
    ...
    for (auto& binding:bindings_) {
        ...
        dispatchBinding(binding,Dispatch::Ready,0.0f,0,input,nodes,totalSteps);
    }
}
...
void GraphVm::tick(float dt,std::uint64_t tick,const GraphInputFrame& input,std::vector<NodeData>& nodes) {
    ...
    for (auto& binding:bindings_) {
        ...
        dispatchBinding(binding,Dispatch::Tick,dt,tick,input,nodes,totalSteps);
    }
}

bool GraphVm::dispatchBinding(Binding& binding,Dispatch dispatch,float dt,std::uint64_t tick,
                              const GraphInputFrame& input,std::vector<NodeData>& nodes,std::size_t& totalSteps) {
    ...
    for (std::uint16_t local=0;local<currentGraph_->nodeCount;++local) {
        ...
        const bool root=dispatch==Dispatch::Ready?opcode==1:(opcode==2 || opcode==3);
        if (!root) continue;
...
bool GraphVm::pureData(std::uint8_t opcode) const { return opcode>=5 && opcode<=12; }
