...
- An independent full-replay verifier for both bundled 524,288-cell KQK/KRK ranked partitions. It reconstructs the complete white-strong canonical no-castling/no-history subdomain from the rule oracle, checks every decisive DTM equation and every closed DRAW continuation, and emits an externally retainable source-bound head only after complete success. The retained heads are evidence, never direct WDL promotion authority.
...
- KQK/KRK are bundled, not the full external 3- through 7-piece corpus. Their complete canonical no-rights/no-history ranked partitions now have independent full-replay heads, and their adapter can certify short finite proofs, but the current v2 fact language cannot reference those cyclic DRAW lemmas. Rights, counters and history must be handled explicitly, and those draws remain `UNKNOWN` to the campaign until a v3 lemma authority, membership proof and migration are implemented.
...
- Large-batch measurements show a local throughput benefit, but no sustained thermal, battery or playing-strength advantage is claimed. The newest timing was captured without exclusive GPU access while another Go-solving task was using shared compute. That contention probably distorted the timing, although the runner did not independently isolate causality; the result is parity evidence and a noisy local snapshot, not a clean performance benchmark.
