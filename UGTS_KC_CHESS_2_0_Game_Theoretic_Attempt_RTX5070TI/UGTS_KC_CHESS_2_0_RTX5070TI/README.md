...
- Expansion now batches one parent’s missing moves into one frontier fsync/SQLite transaction, but authority reconstruction and legal-child coverage still scan the total DAG once per parent. An incremental fact-aware scheduler is required before broad classical expansion is practical. “Complete” in an expansion report means legal move-edge coverage only; optional draw-claim actions remain proof semantics handled by propagation.
...
- A clean rollback to an earlier valid journal suffix cannot be detected without an externally retained head. Canonical head commitments and verification APIs now exist, but the head must actually be stored or signed outside the journal; path-derived locks also do not converge across hardlink aliases.
