...
| Chrono-video log-polar pixel chart | **Active: verified RTX compiler and audited Android source path; physical phone run open** | Separate `UGCVLUT1` maps a rho/theta grid to RGB source pixels as RGBA16UI Q8 addresses. PyTorch CUDA and NumPy use the same explicit integer bilinear operator; exact source PTS and all-pixel UNKNOWN coverage remain in JSON/JSONL. Strict `UGCVPTS1` caches separate the 229-frame authoritative source from the 58-frame already-polar preview. Android verifies all asset/media/timeline/LUT hashes, source/input/output PTS, and applies the LUT only in source mode. Two owned RGBA8 slots prime ordinal zero, prefetch one verified ordinal, publish on the half-open selector boundary and log misses rather than claim them exact. UGLUT2 remains unchanged. | The supplied fixture passed exact source/hash/PTS/bundle checks and CPU/CUDA byte parity; its audited APK is ARM64/MediaNDK-linked with exact packaged assets. No ADB device is attached, so decoder/SurfaceTexture/shader/orientation, display timing, color conversion and performance remain unverified. Explicit LOOP wrap is best-effort; delivered caches are `ONCE_HOLD_LAST`. |
