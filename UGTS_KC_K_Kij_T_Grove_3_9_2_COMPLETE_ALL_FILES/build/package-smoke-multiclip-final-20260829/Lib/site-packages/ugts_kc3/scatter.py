...
# A fixed compatibility namespace keeps Logic Block draws separate from
# Populate Area lineages while reusing the same audited UGTS 4.1 schedule.
REPEATABLE_NUMBER_NAMESPACE = hash64("logic:repeatable-number")
...
def repeatable_number(
    world_number: int,
    pick_number: int,
    smallest: float,
    largest: float,
) -> float:
    """Return one stateless, cross-runtime binary32 number.

    All four inputs enter the binary32 domain before their value constraints
    are checked, matching browser ``Math.fround`` and packed native numbers.
    """

    indexes: list[int] = []
    for label, value in (
        ("World number", world_number),
        ...
        if (
            ...
            raise ScatterError(f"{label} must be a whole number from 0 to 65535")
        try:
            canonical = f32(value)
        except ScatterError as error:
            raise ScatterError(
                f"{label} must be a whole number from 0 to 65535"
            ) from error
        if canonical != math.trunc(canonical) or not 0 <= canonical <= 65535:
            raise ScatterError(f"{label} must be a whole number from 0 to 65535")
        indexes.append(int(canonical))

    endpoints: list[float] = []
    for label, value in (("Smallest", smallest), ("Largest", largest)):
        if (
            isinstance(value, bool)
            or not isinstance(value, (int, float))
            or not math.isfinite(float(value))
    ...
    low, high = endpoints
    if low > high:
        raise ScatterError("Smallest must not be bigger than Largest")

    lineage = stable_id(indexes[0], REPEATABLE_NUMBER_NAMESPACE, indexes[1])
    unit = seed_unit_float(lineage)
    span = float(high) - float(low)
    scaled = float(unit) * span
    try:
        return f32(float(low) + scaled)
    except ScatterError as error:
        raise ScatterError("Repeatable Random Number could not fit its result on this device") from error
