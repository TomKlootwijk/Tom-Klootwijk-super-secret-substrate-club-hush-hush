...
bool RendererGles3::configurePolar(
    ...
    for (std::size_t index=0;index<profiles.size();++index) {
        ...
        if (!std::isfinite(target.rhoMin)||!std::isfinite(target.rhoMax)||
            ...
            KC_LOGI("render substrate polar_requested=%s polar_effective=cpu gpu_instances=0 gpu_profiles=0 gpu_batches=0 cpu_fallbacks=%u animation_fallbacks=0 polar_recipes=%u generated=%u generated_gpu=0 generated_cpu=%u reason=%s bayer=%s levels=%u strength=%.3f",
    ...
    KC_LOGI("render substrate polar_requested=%s polar_effective=%s gpu_instances=%u gpu_profiles=%u gpu_batches=%u cpu_fallbacks=%u animation_fallbacks=%u polar_recipes=%u generated=%u generated_gpu=%u generated_cpu=%u reason=%s bayer=%s levels=%u strength=%.3f",
        ...
        polarReason_.c_str(),bayerRenderModeName(substrate_.bayerMode),
        static_cast<unsigned>(substrate_.levels),substrate_.strength);
    return true;
}
...
void RendererGles3::render(
    ...
    std::uint32_t maxNodes,float time,const GroveJuiceFrame& juice,bool postProcessing
) {
    if (!ready()) return;
    ...
    const auto drawOrdinaryNode=[this,&scene](const NodeData& node) {
    ...
    for (std::size_t recipeIndex=0;
        ...
        for (std::uint32_t localIndex=0;
            ...
            if (gpuCopy) {
                polarGroups_[static_cast<std::size_t>(groupIndex)].visiblePopulationCopies.push_back(
                    polarPopulations.materialize(
                        copyIndex,polarKinematics,nodes,false
                    )
                );
                ++drawn;
                ++visibleGenerated;
            ...
            const auto copy=polarPopulations.materialize(
                copyIndex,polarKinematics,nodes,true
            );
            drawOrdinaryNode(copy.node);
            ++drawn;
            ++visibleGenerated;
            ++visibleGeneratedCpu;
        }
    ...
    if (!polarGroups_.empty() && effectivePolarMode_!=PolarRenderMode::Cpu) {
        ...
        for (GLuint location=2;location<=3;++location) {
