...
An eligible static node may carry one autoplay clip of whole relative transform poses. Export writes
the optional `transform_animations.kcan` (`KCAN392`) asset only when at least one placed node uses the
feature: a 24-byte header, 16-byte sparse node binding and 24 bytes per key. Duration is binary32;
normalized time is unsigned 16-bit; translation, relative quaternion and scale multiplier are
binary16. The C++ loader validates the complete file before content starts.
...
cannot write one transform. This is one autoplay transform clip per eligible node, not multi-clip,
graph-controlled, state-machine or skeletal animation; glTF animation export is also outside this
