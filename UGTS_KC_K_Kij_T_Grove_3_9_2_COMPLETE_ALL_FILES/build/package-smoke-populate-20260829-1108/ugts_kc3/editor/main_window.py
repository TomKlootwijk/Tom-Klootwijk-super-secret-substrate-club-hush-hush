...
class EditorMainWindow(QMainWindow):
    ...
    def _logic_trace_for_current_context(self) -> LogicTraceSnapshot | None:
        ...
        snapshot = self.document.logic_trace(graph_id, owner_id)
        if snapshot is None and owner_id is not None:
            snapshot = self.document.logic_trace(graph_id, None)
        if snapshot is None:
            snapshot = self.document.latest_logic_trace(graph_id)
        return snapshot
