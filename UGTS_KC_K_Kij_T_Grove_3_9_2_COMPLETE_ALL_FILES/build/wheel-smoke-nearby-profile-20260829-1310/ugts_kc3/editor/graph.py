...
class NodePropertiesPanel(QWidget):
    ...
    def _choice_spec(self, node: GraphNode, key: str, value: Any) -> PropertyChoiceSpec | None:
        ...
        if node.template.key == "query.nearest_tag" and key == "origin":
            choices: tuple[tuple[Any, str, str], ...] = ()
        ...
        if node.template.key == "query.nearest_tag" and key == "tag":
            return PropertyChoiceSpec(
                _PORTABLE_TAG_CHOICES,
    ...
    def _choice_changed(
        ...
        if custom_text:
            ...
            if (
                not parsed
                and self.node.template.key == "query.nearest_tag"
                and key == "origin"
            ):
                parsed = None
            elif not parsed:
