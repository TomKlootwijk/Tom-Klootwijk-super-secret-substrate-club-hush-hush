...
constexpr std::uint32_t PlayerTagBit=1u<<0;
...
bool scaleSegmentIsSafe(float left,float right,std::uint8_t easing) {
    ...
    return std::isfinite(extreme)&&extreme!=0.0&&std::signbit(extreme)==std::signbit(left);
}
...
void TransformAnimations::load(
    ...
    try {
        ...
        for (std::uint16_t index=0;index<bindingCount;++index) {
            ...
            require((node.tagMask&PlayerTagBit)==0,"KCAN cannot bind the Player node");
        ...
        for (std::uint32_t index=0;index<totalKeyCount;++index) {
            ...
            require(key.relative.scale.x!=0.0f&&key.relative.scale.y!=0.0f&&
                key.relative.scale.z!=0.0f,"KCAN key scale contains zero");
            keys_.push_back(key);
        }
