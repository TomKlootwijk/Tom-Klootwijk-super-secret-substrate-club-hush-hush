...
void GraphVm::ready(std::vector<NodeData>& nodes) {
    eventCount_=messageCursor_=issueCount_=0; std::size_t totalSteps=0; const GraphInputFrame input{};
...
void GraphVm::tick(float dt,std::uint64_t tick,const GraphInputFrame& input,std::vector<NodeData>& nodes) {
    eventCount_=messageCursor_=issueCount_=0; stepTotalSteps_=0; triggerEventCount_=0; triggerLimitReported_=false;
    for (auto& binding:bindings_) {
        const bool world=binding.sceneNode==WorldBinding;
        if (!binding.enabled || (!world && (binding.sceneNode>=nodes.size() ||
            !nodes[binding.sceneNode].alive || !nodes[binding.sceneNode].active))) continue;
...
void GraphVm::dispatchMessages(float dt,std::uint64_t tick,const GraphInputFrame& input,
    ...
    while (messageCursor_<eventCount_) {
        ...
        for (auto& binding:bindings_) {
            ...
            dispatchBinding(binding,Dispatch::Message,dt,tick,input,nodes,-1,-1,totalSteps);
        }
    }
}
...
bool GraphVm::dispatchBinding(Binding& binding,Dispatch dispatch,float dt,std::uint64_t tick,
    ...
    for (std::uint16_t local=0;local<currentGraph_->nodeCount;++local) {
        ...
        bool root=false;
        if (dispatch==Dispatch::Ready) root=opcode==1;
        else if (dispatch==Dispatch::Tick) root=opcode==2 || opcode==3 || opcode==23;
        else if (dispatch==Dispatch::TriggerEnter) root=opcode==19;
        else if (dispatch==Dispatch::TriggerExit) root=opcode==20;
        else if (dispatch==Dispatch::Message && opcode==25) {
            const auto token=inputs_[nodes_[currentGraph_->nodeStart+local].inputStart];
            const auto& value=values_[token&0xffffu];
            root=strings_[value.index]==currentMessage_.kind;
        }
        if (!root) continue;
