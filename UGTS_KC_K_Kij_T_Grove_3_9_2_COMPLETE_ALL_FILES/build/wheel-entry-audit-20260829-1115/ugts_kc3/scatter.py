...
def scatter_instance(node: Any, group: ScatterGroup, index: int) -> ScatterInstance:
    ...
    translation = tuple(
        f32(float(transform.translation[axis]) + offset[axis]) for axis in range(3)
    )
    scalar = f32(
        population.scale_min
        + _lane(lineage, 4) * (population.scale_max - population.scale_min)
    )
    scale = tuple(f32(float(transform.scale[axis]) * scalar) for axis in range(3))
    yaw = f32(_lane(lineage, 5) * (2.0 * math.pi)) if population.random_yaw else 0.0
    base = quat_normalize(tuple(float(value) for value in transform.rotation))
    if yaw:
        half = float(yaw) * 0.5
