...
std::uint8_t dataOutputs(std::uint8_t opcode) {
    switch (opcode) {
        case 1: return 1;
        case 2: case 3: case 19: case 20: case 23: case 25: return 3;
...
std::uint8_t flowOutputs(std::uint8_t opcode) {
    ...
    if (opcode==1 || opcode==2 || opcode==3 || opcode==19 || opcode==20 || opcode==23 || opcode==25 ||
        (opcode>=13 && opcode<=18) || opcode==26 || opcode==27) return 1;
    return 0;
}
...
bool portableMessageId(std::string_view message) {
    if (message.empty() || message.size()>64 || message.front()<'a' || message.front()>'z') return false;
    return std::all_of(message.begin()+1,message.end(),[](char value) {
...
const char* graphVmErrorName(GraphVmError error) {
    switch (error) {
        ...
        case GraphVmError::EventLimit: return "EventLimit: message queue exceeded 64 events";
...
void GraphVm::load(const std::vector<std::uint8_t>& bytes,std::size_t sceneNodeCount) {
    ...
    executedStamp_.clear(); dataStamp_.clear(); eventCount_=messageCursor_=issueCount_=0;
    dispatchStamp_=evaluationStamp_=0; messageBatchAborted_=false;
    ...
    for (const auto& graph:graphs_) {
        ...
        for (std::uint16_t local=0;local<graph.nodeCount;++local) {
            ...
            if (node.opcode==23) {
                const auto secondsToken=inputs_[node.inputStart];
                const auto repeatToken=inputs_[node.inputStart+1];
                require((secondsToken>>30)==0 && (repeatToken>>30)==0,
                        "KCVG timer inputs must be literals");
                const auto& seconds=values_[secondsToken&0xffffu];
                const auto& repeat=values_[repeatToken&0xffffu];
                require(seconds.tag==ValueTag::Number && seconds.number>0.0f &&
                        seconds.number<=86400.0f,
                        "KCVG timer seconds are invalid");
                require(repeat.tag==ValueTag::Boolean,
                        "KCVG timer repeat is invalid");
            }
            if (node.opcode==25) {
                const auto messageToken=inputs_[node.inputStart];
                require((messageToken>>30)==0,"KCVG message must be a literal");
                const auto& message=values_[messageToken&0xffffu];
...
bool GraphVm::dispatchRoot(const Node& node,Dispatch dispatch) const {
    if (dispatch==Dispatch::Ready) return node.opcode==1;
    if (dispatch==Dispatch::Tick) return node.opcode==2 || node.opcode==3 || node.opcode==23;
    if (dispatch==Dispatch::TriggerEnter) return node.opcode==19;
    if (dispatch==Dispatch::TriggerExit) return node.opcode==20;
    if (node.opcode!=25) return false;
    const auto token=inputs_[node.inputStart];
    const auto& value=values_[token&0xffffu];
    return strings_[value.index]==currentMessage_.kind;
}

bool GraphVm::hasDispatchRoot(const Binding& binding,Dispatch dispatch) const {
    const auto& graph=graphs_[binding.graph];
    for (std::uint16_t local=0;local<graph.nodeCount;++local)
        if (dispatchRoot(nodes_[graph.nodeStart+local],dispatch)) return true;
    return false;
}

void GraphVm::abortMessageBatch() {
    messageBatchAborted_=true;
    // Keep accepted events visible to diagnostics, but discard every pending delivery.
    messageCursor_=eventCount_;
}

void GraphVm::ready(std::vector<NodeData>& nodes) {
    eventCount_=messageCursor_=issueCount_=0; messageBatchAborted_=false;
    std::size_t totalSteps=0; const GraphInputFrame input{};
    for (auto& binding:bindings_) {
        binding.activeStep=0;
        ...
        if (!hasDispatchRoot(binding,Dispatch::Ready)) continue;
        if (totalSteps>=MaxTotalSteps) {
            binding.enabled=false; recordIssue(GraphVmError::TotalStepLimit,binding.graph,0); break;
        }
        if (!dispatchBinding(binding,Dispatch::Ready,0.0f,0,input,nodes,-1,-1,totalSteps) &&
            currentError_==GraphVmError::EventLimit) {
            abortMessageBatch();
            break;
        }
    }
    dispatchMessages(0.0f,0,input,nodes,totalSteps);
}

void GraphVm::tick(float dt,std::uint64_t tick,const GraphInputFrame& input,std::vector<NodeData>& nodes) {
    eventCount_=messageCursor_=issueCount_=0; stepTotalSteps_=0; triggerEventCount_=0;
    triggerLimitReported_=false; messageBatchAborted_=false;
    for (auto& binding:bindings_) {
        const bool world=binding.sceneNode==WorldBinding;
        if (!binding.enabled || (!world && (binding.sceneNode>=nodes.size() ||
            !nodes[binding.sceneNode].alive || !nodes[binding.sceneNode].active))) continue;
        ++binding.activeStep;
        if (!hasDispatchRoot(binding,Dispatch::Tick)) continue;
        if (stepTotalSteps_>=MaxTotalSteps) {
            binding.enabled=false; recordIssue(GraphVmError::TotalStepLimit,binding.graph,0); break;
        }
        if (!dispatchBinding(binding,Dispatch::Tick,dt,tick,input,nodes,-1,-1,stepTotalSteps_) &&
            currentError_==GraphVmError::EventLimit) {
            abortMessageBatch();
            break;
        }
...
void GraphVm::finishStep(float dt,std::uint64_t tick,const GraphInputFrame& input,
                         std::vector<NodeData>& nodes) {
    dispatchMessages(dt,tick,input,nodes,stepTotalSteps_);
}

void GraphVm::dispatchMessages(float dt,std::uint64_t tick,const GraphInputFrame& input,
                               std::vector<NodeData>& nodes,std::size_t& totalSteps) {
    if (messageBatchAborted_) return;
    while (!messageBatchAborted_ && messageCursor_<eventCount_) {
        currentMessage_=events_[messageCursor_++];
        for (auto& binding:bindings_) {
            const bool world=binding.sceneNode==WorldBinding;
            if (!binding.enabled || (!world && (binding.sceneNode>=nodes.size() ||
                !nodes[binding.sceneNode].alive || !nodes[binding.sceneNode].active))) continue;
            if (currentMessage_.target>=0 && !world &&
                binding.sceneNode!=static_cast<std::uint32_t>(currentMessage_.target)) continue;
            if (!hasDispatchRoot(binding,Dispatch::Message)) continue;
            if (totalSteps>=MaxTotalSteps) {
                recordIssue(GraphVmError::TotalStepLimit,binding.graph,0);
                return;
            }
            if (!dispatchBinding(binding,Dispatch::Message,dt,tick,input,nodes,-1,-1,totalSteps) &&
                currentError_==GraphVmError::EventLimit) {
                abortMessageBatch();
                return;
            }
...
void GraphVm::trigger(bool entering,std::uint32_t sensor,std::uint32_t player,float dt,
    ...
    if (messageBatchAborted_ || bindings_.empty() || sensor>=nodes.size() || player>=nodes.size()) return;
    ...
    for (auto& binding:bindings_) {
        ...
        if (!world && (binding.sceneNode>=nodes.size() ||
            !nodes[binding.sceneNode].alive || !nodes[binding.sceneNode].active)) continue;
        if (!hasDispatchRoot(binding,dispatch)) continue;
        if (stepTotalSteps_>=MaxTotalSteps) {
            recordIssue(GraphVmError::TotalStepLimit,binding.graph,0);
            break;
        }
...
bool GraphVm::dispatchBinding(Binding& binding,Dispatch dispatch,float dt,std::uint64_t tick,
    ...
    currentActiveStep_=binding.activeStep;
    ...
    for (std::uint16_t local=0;local<currentGraph_->nodeCount;++local) {
        if (!dispatchRoot(nodes_[currentGraph_->nodeStart+local],dispatch)) continue;
...
bool GraphVm::executeNode(std::uint16_t local,bool queued,std::size_t depth,std::uint8_t& flowMask) {
    ...
    switch (node.opcode) {
        ...
        case 23: {
            float seconds=0.0f;
            bool repeat=false;
            if (!numberValue(input[0],seconds) || !boolValue(input[1],repeat))
                return fail(GraphVmError::TypeMismatch,local);
            if (!std::isfinite(seconds) || seconds<=0.0f || seconds>86400.0f)
                return fail(GraphVmError::InvalidTimerDuration,local);
            const float stepDt=roundedFloat(currentDt_);
            if (!std::isfinite(stepDt) || stepDt<=0.0f)
                return fail(GraphVmError::InvalidTimerStep,local);
            const float ratio=roundedFloat(seconds/stepDt);
            if (!std::isfinite(ratio) || ratio<=0.0f ||
                ratio>=static_cast<float>(std::numeric_limits<std::uint64_t>::max()))
                return fail(GraphVmError::InvalidTimerStep,local);
            const auto period=std::max<std::uint64_t>(
                1,static_cast<std::uint64_t>(std::ceil(ratio))
            );
            const auto step=currentActiveStep_;
            bool ringing=false;
            std::uint64_t rings=0,remainingSteps=0;
            if (repeat) {
                const auto remainder=step%period;
                ...
                remainingSteps=step>=period?0:period-step;
            }
            const float count=roundedFloat(static_cast<float>(rings));
            const float remaining=remainingSteps==0?0.0f:roundedFloat(
                roundedFloat(static_cast<float>(remainingSteps))*stepDt
            );
            if (!std::isfinite(count) || !std::isfinite(remaining))
                return fail(GraphVmError::InvalidTimerStep,local);
            output[0]=number(count);
            output[1]=number(remaining);
            output[2]=entity();
            flowMask=ringing?1u:0u;
            return true;
        }
        case 24: {
            const auto origin=entityValue(input[0],true);
            ...
            if (!std::isfinite(radius) || radius<0.0f || !std::isfinite(radiusSquared))
                return fail(GraphVmError::InvalidSearchRadius,local);
            if (input[3].tag!=ValueTag::Vec4) return fail(GraphVmError::TypeMismatch,local);

            const float axisX=input[3].vector[0];
            const float axisY=input[3].vector[1];
            const float axisZ=input[3].vector[2];
            const float minimumCosine=input[3].vector[3];
            const float axisXX=roundedFloat(axisX*axisX);
            const float axisYY=roundedFloat(axisY*axisY);
            const float axisZZ=roundedFloat(axisZ*axisZ);
            float axisSquared=roundedFloat(axisXX+axisYY);
            axisSquared=roundedFloat(axisSquared+axisZZ);
            if (!std::isfinite(axisSquared) || axisSquared<=0.0f ||
                !std::isfinite(minimumCosine) || minimumCosine < -1.0f || minimumCosine > 1.0f)
                return fail(GraphVmError::InvalidSearchCone,local);
            const float axisLength=roundedFloat(std::sqrt(axisSquared));
            const float normalizedAxisX=roundedFloat(axisX/axisLength);
            const float normalizedAxisY=roundedFloat(axisY/axisLength);
            const float normalizedAxisZ=roundedFloat(axisZ/axisLength);
            if (!std::isfinite(axisLength) || axisLength<=0.0f ||
                !std::isfinite(normalizedAxisX) || !std::isfinite(normalizedAxisY) ||
                !std::isfinite(normalizedAxisZ))
                return fail(GraphVmError::InvalidSearchCone,local);
            ...
            constexpr float DirectionDenominatorMinimum=1.0e-6f;
            std::uint32_t best=std::numeric_limits<std::uint32_t>::max();
            float bestSquared=0.0f;
            for (std::uint32_t index=0;index<currentNodes_->size();++index) {
                if (index==originIndex) continue;
                ...
                const float distance=roundedFloat(std::sqrt(squared));
                const float denominator=std::max(distance,roundedFloat(DirectionDenominatorMinimum));
                const float directionX=roundedFloat(dx/denominator);
                const float directionY=roundedFloat(dy/denominator);
                const float directionZ=roundedFloat(dz/denominator);
                const float cosineX=roundedFloat(directionX*normalizedAxisX);
                const float cosineY=roundedFloat(directionY*normalizedAxisY);
                const float cosineZ=roundedFloat(directionZ*normalizedAxisZ);
                float cosineToAxis=roundedFloat(cosineX+cosineY);
                cosineToAxis=roundedFloat(cosineToAxis+cosineZ);
                if (!std::isfinite(distance) || !std::isfinite(cosineToAxis))
                    return fail(GraphVmError::NonFinite,local);
                if (cosineToAxis<minimumCosine) continue;
        ...
        case 25:
            output[0]=indexedEntity(currentMessage_.source);
            output[1]=indexedEntity(currentMessage_.target);
            output[2]=entity();
            flowMask=1;
            return true;
        case 26: {
