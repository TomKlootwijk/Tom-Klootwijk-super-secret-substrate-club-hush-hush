...
#define KC_LOGI(...) __android_log_print(ANDROID_LOG_INFO,"UGTS-KC-GROVE",__VA_ARGS__)
#define KC_LOGE(...) __android_log_print(ANDROID_LOG_ERROR,"UGTS-KC-GROVE",__VA_ARGS__)
...
bool Engine::initializeWindow() {
    ...
    try {
        scene_=parseScenePack(readAsset("grove_scene.kc3d"));
    } catch (...) {
    ...
    KC_LOGI("UGTS-KC 3.9.2 K-Kij-T Grove profile=%s grove=%s quality=%s fps=%u scale=%.2f model=%s gpu=%s ram=%uMB juice=%.2f",
