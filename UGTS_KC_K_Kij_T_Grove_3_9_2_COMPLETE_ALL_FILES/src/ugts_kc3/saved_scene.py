...
The materializer is pure, deterministic and idempotent. Saved Scene placements
intentionally become flat runtime nodes; they are linked authoring groups, not
retained prefab instances or ordinary parent/child transform edges.
"""
...
class SavedSceneNode3D:
    ...
    def __post_init__(self) -> None:
        ...
        # SavedSceneNode3D owns the definition-local parent edge.  Keeping the
        # ordinary runtime ``Node3DRecord.parent_id`` as well would encode the
        # same relationship in two namespaces; after instance ids are rebased,
        # that stale local id becomes a missing ordinary scene parent.  The
        # nested payload is therefore always a detached flat record.
        if (
            getattr(detached, "id", None) != local_id
            or getattr(detached, "parent_id", None) is not None
        ):
            detached = replace(detached, id=local_id, parent_id=None)
