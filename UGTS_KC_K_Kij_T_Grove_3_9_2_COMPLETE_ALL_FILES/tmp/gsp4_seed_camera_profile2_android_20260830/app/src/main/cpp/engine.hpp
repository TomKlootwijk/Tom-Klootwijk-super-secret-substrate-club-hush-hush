...
public:
    ...
    bool initializeWindow(std::int64_t steadyNowNanoseconds);
    void terminateWindow();
    void setFocused(bool value,std::int64_t steadyNowNanoseconds);
    bool focused() const { return focused_; }
    bool ready() const { return renderer_.ready(); }
    int handleInput(AInputEvent* event);
    void frame(float dt,std::int64_t steadyNowNanoseconds);
private:
    ...
    ChronoVideoLut chronoVideoLut_;
#if UGTS_KC_GSP4_SEED_CAMERA
    ChronoSceneBindings chronoSceneBindings_;
    ChronoSeedCaptureSession chronoSeedCapture_;
#endif
