...
void validateOutputBasename(const std::string& value) {
    ...
    require(value.ends_with(".ugsp4c"),
        "KCCH392 app-private stream basename must end in .ugsp4c");
}
...
void ChronoSceneBindings::load(
    ...
    for (std::uint32_t index=0;index<recordCount;++index) {
        ...
        binding.storage=static_cast<ChronoSceneStorage>(storage);
        ...
        const auto autostart=reader.u8();
        require(autostart<=1u,"KCCH392 autostart must be a strict boolean");
        binding.autostart=autostart!=0u;
        ...
        if (binding.mode==ChronoSceneMode::Recorder) {
            ...
            require(refs.cameraLength==0u,"KCCH392 player must not own a Camera2 id");
            if (storage==1u) {
                require(binding.sourceAssetBytes==0u && digestIsZero(binding.sourceAssetSha256),
                    "KCCH392 app-private player must not claim a packaged receipt");
                require(refs.outputLength>0u && refs.assetLength==0u,
                    "KCCH392 app-private player stream name is absent");
            } else {
                require(binding.sourceAssetBytes>0u && !digestIsZero(binding.sourceAssetSha256),
                    "KCCH392 packaged player source receipt is absent");
                require(refs.outputLength==0u && refs.assetLength>0u,
                    "KCCH392 packaged player string fields are invalid");
            }
        }
    ...
    for (std::size_t index=0;index<records_.size();++index) {
        ...
        if (binding.storage==ChronoSceneStorage::AppPrivateGsp4Seed)
            validateOutputBasename(binding.outputBasename);
        else require(
    ...
    const auto* recorderBinding=recorder();
    for (const auto& binding:records_) {
        if (binding.mode!=ChronoSceneMode::Player ||
            binding.storage!=ChronoSceneStorage::AppPrivateGsp4Seed) continue;
        require(recorderBinding,"KCCH392 app-private player has no recorder authority");
        require(
            binding.outputBasename==recorderBinding->outputBasename &&
            binding.width==recorderBinding->width && binding.height==recorderBinding->height &&
            binding.fps==recorderBinding->fps &&
            binding.rootSeed==recorderBinding->rootSeed &&
            binding.recipeSeed==recorderBinding->recipeSeed &&
            binding.uglutResolution==recorderBinding->uglutResolution &&
            binding.uglut2Sha256==recorderBinding->uglut2Sha256 &&
            binding.r0==recorderBinding->r0 && binding.rhoMin==recorderBinding->rhoMin &&
            binding.rhoMax==recorderBinding->rhoMax &&
            binding.coreRadius==recorderBinding->coreRadius,
            "KCCH392 app-private player disagrees with its recorder profile"
        );
    }
}
