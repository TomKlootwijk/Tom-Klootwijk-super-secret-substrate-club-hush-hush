...
bool RendererGles3::initialize(
    ...
    if (!chronoVideoPlayer_.initialize(
            activity,assets,chronoVideoLut,chronoVideoLutTexture_
        )) KC_LOGE(
            "chrono_video=FAILED_CLOSED preview_promotion=false "
            "ordinary_editable_scene_continues=true"
        );
#else
    (void)activity;
    (void)chronoVideoLut;
...
void RendererGles3::render(
    ...
    std::uint64_t fixedTick,float time,std::int64_t steadyNowNanoseconds,
...
#if !UGTS_KC_GSP4_SEED_CAMERA
    chronoVideoPlayer_.updateAndStage(steadyNowNanoseconds);
#else
...
#if !UGTS_KC_GSP4_SEED_CAMERA
    chronoVideoPlayer_.drawBackground(internalWidth_,internalHeight_);
#endif
