...
bool ChronoCaptureFrameQueue::waitForCaptured(std::chrono::milliseconds timeout) {
    std::unique_lock lock(mutex_);
    return condition_.wait_for(lock,timeout,[this]{
        return stats_.error!=ChronoCaptureError::None ||
            std::any_of(slots_.begin(),slots_.end(),[](const Slot& slot){
                return slot.state==SlotState::Captured;
            });
    });
}

void ChronoCaptureFrameQueue::wake() { condition_.notify_all(); }
