...
constexpr std::uint32_t MaxAnimatedNodes=64;
constexpr std::uint32_t MaxClips=256;
constexpr std::uint32_t MaxClipsPerNode=16;
...
constexpr std::size_t V1BindingBytes=16;
constexpr std::size_t V2BindingBytes=24;
...
std::uint64_t animationClipHash(std::string_view clipId) {
    ...
    for (const unsigned char value:clipId) {
...
void TransformAnimations::clear() {
    clips_.clear(); controllers_.clear(); keys_.clear();
}
...
void TransformAnimations::load(
    ...
    try {
        ...
        require(version==1u||version==2u,"unsupported KCAN version");
        const auto clipCount=reader.u16();
        require(reader.u16()==0,"KCAN header reserved field is nonzero");
        const auto totalKeyCount=reader.u32();
        const auto maximumClips=version==1u?MaxAnimatedNodes:MaxClips;
        require(clipCount>=1&&clipCount<=maximumClips,"KCAN clip count limit exceeded");
        require(totalKeyCount>=clipCount&&totalKeyCount<=MaxKeys,"KCAN key count limit exceeded");
        const auto bindingBytes=version==1u?V1BindingBytes:V2BindingBytes;
        const auto expectedSize=HeaderBytes+
            static_cast<std::size_t>(clipCount)*bindingBytes+
            static_cast<std::size_t>(totalKeyCount)*KeyBytes;
        require(bytes.size()==expectedSize,"KCAN byte length does not match its counts");

        clips_.reserve(clipCount);
        controllers_.reserve(std::min<std::uint32_t>(clipCount,MaxAnimatedNodes));
        std::uint32_t previousNode=0,expectedFirstKey=0;
        std::uint64_t previousHash=0;
        bool havePrevious=false;
        for (std::uint16_t index=0;index<clipCount;++index) {
            Clip clip;
            clip.sceneNode=reader.u32();
            clip.hash=version==1u?animationClipHash("main"):reader.u64();
            clip.duration=reader.f32(); clip.firstKey=reader.u32();
            clip.keyCount=reader.u16(); clip.loop=reader.u8();
            const auto flags=reader.u8();
            if (version==1u) require(flags==0,"KCAN v1 binding reserved field is nonzero");
            else require((flags&~1u)==0,"KCAN v2 clip flags are invalid");
            clip.autoplay=version==1u||(flags&1u)!=0;
            require(clip.sceneNode<nodes.size(),"KCAN clip node index is invalid");
            if (havePrevious) {
                const bool ordered=clip.sceneNode>previousNode||
                    (clip.sceneNode==previousNode&&clip.hash>previousHash);
                require(ordered,"KCAN clips are not sparse-canonical");
            }
            havePrevious=true; previousNode=clip.sceneNode; previousHash=clip.hash;
            require(std::isfinite(clip.duration)&&clip.duration>=1.0f/60.0f&&
                clip.duration<=120.0f,"KCAN duration is outside the supported range");
            require(clip.firstKey==expectedFirstKey,"KCAN key ranges are not contiguous");
            require(clip.keyCount>=1&&clip.keyCount<=MaxKeysPerBinding,
                "KCAN binding key count limit exceeded");
            expectedFirstKey+=clip.keyCount;
            require(expectedFirstKey<=totalKeyCount,"KCAN binding key range is invalid");
            require(clip.loop<=2,"KCAN loop mode is invalid");
            const auto& node=nodes[clip.sceneNode];
            ...
            if (controllers_.empty()||controllers_.back().sceneNode!=clip.sceneNode) {
                require(controllers_.size()<MaxAnimatedNodes,
                    "KCAN animated node count limit exceeded");
                Controller controller;
                controller.sceneNode=clip.sceneNode;
                controller.firstClip=index;
                controller.base={node.translation,normalize(node.rotation),node.scale};
                controllers_.push_back(controller);
            }
            auto& controller=controllers_.back();
            require(controller.clipCount<MaxClipsPerNode,
                "KCAN clips-per-node limit exceeded");
            if (clip.autoplay) {
                require(controller.activeClip==Controller::NoClip,
                    "KCAN node has more than one autoplay clip");
                controller.activeClip=index;
        ...
        for (const auto& clip:clips_) {
            const auto begin=static_cast<std::size_t>(clip.firstKey);
            const auto end=begin+clip.keyCount;
...
const TransformAnimations::Clip* TransformAnimations::activeClip(
    const Controller& controller
) const {
    if (controller.activeClip==Controller::NoClip||controller.activeClip>=clips_.size())
        return nullptr;
    return &clips_[controller.activeClip];
}

TransformAnimations::Controller* TransformAnimations::controller(std::uint32_t sceneNode) {
    const auto found=std::lower_bound(
        controllers_.begin(),controllers_.end(),sceneNode,
        [](const Controller& value,std::uint32_t target) { return value.sceneNode<target; }
    );
    return found!=controllers_.end()&&found->sceneNode==sceneNode?&*found:nullptr;
}

const TransformAnimations::Controller* TransformAnimations::controller(
...
bool TransformAnimations::owns(std::uint32_t sceneNode) const {
    return controller(sceneNode)!=nullptr;
}

TransformAnimations::Pose TransformAnimations::sample(
    const Clip& clip,double elapsed
) const {
    const auto first=static_cast<std::size_t>(clip.firstKey);
    const auto last=first+clip.keyCount-1u;
    double local=std::max(0.0,elapsed);
    const double duration=clip.duration;
    if (clip.loop==0) local=std::min(duration,local);
    else if (clip.loop==1) local=std::fmod(local,duration);
    else {
...
void TransformAnimations::compose(
    const Controller& controller,std::vector<NodeData>& nodes
) const {
    const auto* clip=activeClip(controller);
    if (!clip) return;
    auto& node=nodes[controller.sceneNode];
    if (!node.alive||!node.active) return;
    const auto relative=sample(*clip,controller.elapsed);
    const Vec3 scale{
        controller.base.scale.x*relative.scale.x,
        controller.base.scale.y*relative.scale.y,
        controller.base.scale.z*relative.scale.z,
    };
    ...
    node.translation=controller.base.translation+relative.translation;
    node.rotation=normalize(multiply(controller.base.rotation,relative.rotation));
    node.scale=scale;
}

void TransformAnimations::restore(
    const Controller& controller,std::vector<NodeData>& nodes
) const {
    auto& node=nodes[controller.sceneNode];
    node.translation=controller.base.translation;
    node.rotation=controller.base.rotation;
    node.scale=controller.base.scale;
}

void TransformAnimations::compose(std::vector<NodeData>& nodes) const {
    for (const auto& controller:controllers_) compose(controller,nodes);
}
...
void TransformAnimations::tick(float dt,std::vector<NodeData>& nodes) {
    ...
    for (auto& controller:controllers_) {
        const auto* clip=activeClip(controller);
        if (clip&&controller.playing) {
            controller.elapsed+=static_cast<double>(dt);
            require(std::isfinite(controller.elapsed),"transform animation clock overflowed");
            if (clip->loop==0&&controller.elapsed>=clip->duration) {
                controller.elapsed=clip->duration;
                controller.playing=false;
            }
        }
        // Clocks advance even while their scene node is inactive. A held or
        // completed clip is still composed when that node becomes active again.
        compose(controller,nodes);
    }
}

AnimationControlResult TransformAnimations::play(
    std::uint32_t sceneNode,std::uint64_t clipHash,bool restart,
    std::vector<NodeData>& nodes
) {
    auto* target=controller(sceneNode);
    if (!target) return AnimationControlResult::MissingController;
    const auto begin=clips_.begin()+target->firstClip;
    const auto end=begin+target->clipCount;
    const auto found=std::lower_bound(
        begin,end,clipHash,
        [](const Clip& clip,std::uint64_t hash) { return clip.hash<hash; }
    );
    if (found==end||found->hash!=clipHash) return AnimationControlResult::MissingClip;
    const auto selected=static_cast<std::uint16_t>(found-clips_.begin());
    if (target->activeClip!=selected||restart) target->elapsed=0.0;
    target->activeClip=selected;
    target->playing=true;
    // Graph actions execute after the animation tick. Compose time zero now so
    // the selected clip is visible this frame and advances on the next tick.
    compose(*target,nodes);
    return AnimationControlResult::Ok;
}

AnimationControlResult TransformAnimations::stop(
    ...
    if (reset) {
        target->activeClip=Controller::NoClip;
        target->elapsed=0.0;
