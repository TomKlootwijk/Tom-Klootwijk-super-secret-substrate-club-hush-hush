...
def build_story() -> list:
    ...
    story.extend([
        ...
        Spacer(1, 7 * mm),
        p("Corrected UGTOMS-first engineering report v0.3", "cover_meta"),
        p("Draft profiles: UGTOMS-CSO-0.1-PROPOSAL (scene) + UGTOMS-HCO-0.1-PROPOSAL (human specialization)", "cover_meta"),
        ...
        Spacer(1, 13 * mm),
        p("Status: architecture and implementation strategy. The UGTOMS common envelope and these profiles are not implemented universal formats. Current Grove has no Chrono-BRACE scene compiler, skeletal character animation, dynamic surface staging or general scene/object residual codec. The supplied clip has been inspected and probed, but no static scene surface, moving object or person mesh has been accepted as physical truth.", "cover_meta"),
        Spacer(1, 5 * mm),
    ...
    story.extend([
        ...
        callout(
            "Name and status",
            "UGTOMS-CSO-0.1-PROPOSAL means Chrono Scene Observation; UGTOMS-HCO-0.1-PROPOSAL specializes a promoted object when the target is human. Both are 3D plus time, not a fourth spatial dimension. They are design profiles, not implemented codecs, standards, registrations, or universal-interoperability claims. [R01, R02, R10]",
            GOLD,
    ...
    story.extend([
        ...
        two_column(
            ...
            [p("What is not claimed", "h2")] + bullet_lines([
                ...
                "Current Grove implementation of CSO, Chrono-BRACE or HCO.",
            ]),
        ),
        PageBreak(),
    ])
    ...
    story.extend([
        ...
        data_table(
            ...
            [
                ...
                ["Internal runtime", "Strict sibling CSO/object sidecar only after promotion gates", "CONDITIONAL. Potential seek/runtime gains; name/layout remain TBD and unfrozen."],
    ...
    story.extend([
        ...
        data_table(
            ...
            [
                ["1 - HIGH", "Camera matchmove plus partial static environment", "Static support can use guarded evidence outside moving-object branches; open mesh/points/voxels are useful without hidden completion.", "Beat conventional SfM/SLAM export on inspectability, bitemporal replay or editable provenance at equal reprojection error."],
                ["2 - HIGH", "Scene/object compiler onto authored proxies", "Separates camera, static set and moving objects; proxy closure gives editable runtime assets without hidden-shape claims.", "Beat or match conventional glTF scene/animation workflow on error, correction time, seek and bytes."],
                ["3 - HIGH", "Human performance/HCO specialization", "Articulated root/part chronology supports animation blocking, replay ghosts and VFX while body topology stays authored.", "Correction time and replay determinism must improve over video reference; no completed anatomy may commit."],
                ["4 - MEDIUM", "Hybrid game/VFX scene", "Combines supported static/dynamic residuals with proxy topology, collision and per-field provenance.", "Visible value must exceed tool/runtime complexity; downstream raster cannot self-certify."],
    ...
    story.extend([
        ...
        callout(
            ...
            "PASS: byte-exact source binding, decoded chronology, hashing, six-frame inspection and a deterministic unsegmented motion diagnostic. FAIL-CLOSED FOR LITERAL 3D: the clip exposes no intrinsics, distortion, exposure/shutter model, depth bound or metric anchor. NOT IMPLEMENTED/VERIFIED: Chrono-BRACE contraction, accepted static-scene support, any moving object's 3D support, a performer's HCO state, proxy drive, physics or runtime materialization. The current result is a bounded test contract, not a reconstructed scene/person.",
            RED,
            colors.HexColor("#FCEDEF"),
        ),
        Spacer(1, 3 * mm),
    ...
    story.extend([
        ...
        Spacer(1, 4 * mm),
        p("The root TODO.md is updated with this sequence, ownership rules, benchmark matrix and stop conditions. It remains a backlog, not evidence that the profile is implemented.", "body"),
        callout(
    ...
    story.extend([
        ...
        data_table(
            ...
            [
                ...
                ["Implemented CSO/Chrono-BRACE/HCO support in current Grove", "Current runtime has no observation compiler, dynamic surface staging, human profile, skeletal animation or general scene/object residual codec."],
                ...
                ["Physical-device proof for scene/object workload", "Existing Poco evidence belongs to bounded polar Grow rendering and cannot transfer to CSO/Chrono-BRACE/HCO."],
            ],
            [63 * mm, 111 * mm],
        ),
        Spacer(1, 4 * mm),
    ...
    story.extend([
        ...
        callout(
            "Evidence hierarchy",
            "Current code and contracts govern implemented behavior. Preserved build/device records support only their exact workload. Proposed UGTOMS, CSO, Chrono-BRACE and HCO claims remain architecture. Legacy PDFs and attached documents inform chronology but do not override the user's request or current source state.",
            BLUE,
            colors.HexColor("#EDF3FA"),
        ),
        PageBreak(),
