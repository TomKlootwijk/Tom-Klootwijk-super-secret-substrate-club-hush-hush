...
#if !UGTS_KC_GSP4_SEED_CAMERA
        chronoVideoLut_.load(readAsset("chrono/polar_lut.ugcv1"));
        if (chronoVideoLut_.present) KC_LOGI(
            "UGCVLUT1 verified: source=%ux%u polar=%ux%u authority=derived_cache",
            chronoVideoLut_.sourceWidth,chronoVideoLut_.sourceHeight,
            chronoVideoLut_.thetaBins,chronoVideoLut_.rhoBins
        );
#else
        chronoSceneBindings_.load(readAsset("chrono_bindings.kcch"),nodes_.size());
        if (const auto* recorder=chronoSceneBindings_.recorder()) {
            const auto lutPath="chrono/uglut2/"+digestHex(recorder->uglut2Sha256)+".uglut2";
            auto literalUglut2=readAsset(lutPath.c_str());
            if (literalUglut2.empty())
                throw std::runtime_error("KCCH392 recorder literal UGLUT2 asset is absent");
            if (!chronoSeedCapture_.configure(
                    app_->activity,*recorder,std::move(literalUglut2)))
                throw std::runtime_error("KCCH392 recorder runtime configuration failed");
            KC_LOGI(
                "KCCH392 recorder node=%u mode=RECORDER ordinary_scene_owner=true "
                "global_fullscreen=false autostart=%s",
                recorder->nodeIndex,recorder->autostart?"true":"false"
            );
        }
#endif
...
bool Engine::initializeWindow(std::int64_t steadyNowNanoseconds) {
    ...
    renderer_.setChronoFocused(focused_,steadyNowNanoseconds);
#if UGTS_KC_GSP4_SEED_CAMERA
    if (chronoSeedCapture_.configured() && chronoSeedCapture_.autostart())
        chronoSeedCapture_.requestStart();
#endif
...
void Engine::terminateWindow() {
#if UGTS_KC_GSP4_SEED_CAMERA
    if (chronoSeedCapture_.state()==ChronoSeedCaptureState::Capturing)
        chronoSeedCapture_.stopAndFinalize();
#endif
    renderer_.shutdown();
}

void Engine::setFocused(bool value,std::int64_t steadyNowNanoseconds) {
    focused_=value;
    renderer_.setChronoFocused(value,steadyNowNanoseconds);
#if UGTS_KC_GSP4_SEED_CAMERA
    if (!value && chronoSeedCapture_.state()==ChronoSeedCaptureState::Capturing)
        chronoSeedCapture_.stopAndFinalize();
    else if (value && chronoSeedCapture_.configured() && chronoSeedCapture_.autostart() &&
        chronoSeedCapture_.state()==ChronoSeedCaptureState::Configured)
        chronoSeedCapture_.requestStart();
#endif
}
...
void Engine::frame(float dt,std::int64_t steadyNowNanoseconds) {
#if UGTS_KC_GSP4_SEED_CAMERA
    chronoSeedCapture_.pump();
#endif
    ...
    renderer_.render(
        ...
        quality.maxVisibleNodes,remainingParticleBudget,fixedTick_,time_,steadyNowNanoseconds,
        juice,usePost
    );
}
...
int Engine::handleInput(AInputEvent* event) {
    ...
    if (type==AINPUT_EVENT_TYPE_KEY) {
        ...
        if (key==AKEYCODE_W || key==AKEYCODE_DPAD_UP) moveZ_=down?-1.0f:0.0f;
            ...
            if (down && !repeated && chronoSeedCapture_.configured()) {
                const auto state=chronoSeedCapture_.state();
                if (state==ChronoSeedCaptureState::Capturing ||
                    state==ChronoSeedCaptureState::AwaitingPermission)
                    chronoSeedCapture_.stopAndFinalize();
                else chronoSeedCapture_.requestStart();
            }
        }
#endif
        else return 0;
