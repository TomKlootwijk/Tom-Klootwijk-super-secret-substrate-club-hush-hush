...
bool ChronoVideoPlayer::createGlResources(
    ...
    glGenTextures(1,&stagingTexture_);
    ...
    glGenFramebuffers(1,&stagingFramebuffer_);
    glBindFramebuffer(GL_FRAMEBUFFER,stagingFramebuffer_);
    glFramebufferTexture2D(
        GL_FRAMEBUFFER,GL_COLOR_ATTACHMENT0,GL_TEXTURE_2D,stagingTexture_,0);
    const auto complete=glCheckFramebufferStatus(GL_FRAMEBUFFER)==GL_FRAMEBUFFER_COMPLETE;
    const auto error=glGetError();
    glBindFramebuffer(GL_FRAMEBUFFER,0);
    glBindTexture(GL_TEXTURE_2D,0);
    if (!externalTexture_ || !stagingTexture_ || !stagingFramebuffer_ || !complete || error!=GL_NO_ERROR) {
        KC_LOGE("chrono GL resource creation failed: complete=%s error=0x%x",
            complete?"true":"false",error);
        return false;
    }
    KC_LOGI("chrono owned staging raster=%ux%u format=RGBA8 external_filter=NEAREST",
...
void ChronoVideoPlayer::shutdown() {
    ...
    if (eglGetCurrentContext()!=EGL_NO_CONTEXT) {
        if (stagingFramebuffer_) glDeleteFramebuffers(1,&stagingFramebuffer_);
        if (stagingTexture_) glDeleteTextures(1,&stagingTexture_);
    ...
    stagingFramebuffer_=stagingTexture_=externalTexture_=0;
    ...
    havePreviousTarget_=loopResetPending_=false;
...
void ChronoVideoPlayer::stageExternalFrame(const std::array<float,16>& transform) {
...
void ChronoVideoPlayer::releaseHeldOutput(std::size_t targetOrdinal) {
    if (!heldOutput_.valid || surfaceFrameAwaited_ || heldOutput_.ordinal>targetOrdinal) return;
    const auto held=heldOutput_;
    heldOutput_={};
    ++outputOrdinal_;
    if (held.ordinal<targetOrdinal) {
        ...
        if (decoderCatchupDropCount_==1u || (decoderCatchupDropCount_%60u)==0u)
            KC_LOGI("chrono presentation catchup discarded=%llu last_ordinal=%zu target=%zu",
                static_cast<unsigned long long>(decoderCatchupDropCount_),held.ordinal,targetOrdinal);
    } else {
...
void ChronoVideoPlayer::dequeueDecoder(std::size_t targetOrdinal) {
    releaseHeldOutput(targetOrdinal);
...
void ChronoVideoPlayer::updateAndStage(std::int64_t steadyNowNanoseconds) {
    ...
    try {
        ...
        const auto target=timeline_.selectForElapsedNanoseconds(
            elapsedNanoseconds(steadyNowNanoseconds));
        if (timeline_.loop() && havePreviousTarget_ && target<previousTargetOrdinal_)
            loopResetPending_=true;
        previousTargetOrdinal_=target;
...
void ChronoVideoPlayer::drawBackground(int width,int height) const {
    if (!active() || stagedFrameCount_==0u || !stagingTexture_ || width<=0 || height<=0) return;
    glViewport(0,0,width,height);
    glDisable(GL_DEPTH_TEST); glDisable(GL_CULL_FACE);
    glUseProgram(backgroundProgram_);
    glActiveTexture(GL_TEXTURE0); glBindTexture(GL_TEXTURE_2D,stagingTexture_);
