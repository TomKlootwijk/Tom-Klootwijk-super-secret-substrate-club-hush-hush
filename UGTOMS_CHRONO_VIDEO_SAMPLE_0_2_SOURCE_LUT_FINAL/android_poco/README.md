...
SurfaceTexture callback, then copy into an owned RGBA8 texture. Ordinal zero is staged before the
integer steady clock starts; the cache's half-open source intervals select subsequent frames. The
default is `ONCE_HOLD_LAST`, while looping occurs only when the cache carries its explicit loop flag.
